...
var Book_v5 = {
    ...
    media_queries:function (){
       ...
       if( xMax_center > xMin_right || xMax_left > xMin_center  ){
		 ...
		 $('#fb5 #fb5-logo').css('top','-20px');
		 $('#fb5 #fb5-logo').css('left','-2px');
		 $('#fb5 #fb5-logo img').css('width','100px');
		 $('#fb5 #fb5-logo img').css('height','20px');
		 $('#fb5 #fb5-center').css('position','initial');
       }else{
...
function contact_form() {
	...
	$('#fb5-contact button[type="submit"]').click(function() {
        ...
        var emailform = Book_v5.config['email_form'];
		var dataString = 'name='+ name + '&email=' + email + '&message=' + message+'&emailform='+emailform;  
